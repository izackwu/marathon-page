...
export function MapView({ results }: MapViewProps) {
  const groupedResults = groupBy(results, r => `${r.location.coordinates.lat},${r.location.coordinates.lng}`);
  const bounds = L.latLngBounds(Object.values(groupedResults).map(races => 
    [races[0].location.coordinates.lat, races[0].location.coordinates.lng]
  ));
                ...
                <Popup>
                  <div className="text-sm">
                    <div className="flex items-center gap-2 mb-3">
                      {React.createElement(getCountryFlag(races[0].location.country, races[0].location.name), {
                        className: "w-4 h-4"
                      })}
                      <div>
                    ...
                    {sortedRaces.map((race) => (
                      <div key={`${race.date}-${race.name}`} className="border-t pt-2 first:border-t-0 first:pt-0">
                        ...
                        <div className="font-medium">{formatDuration(race.finishTime)}</div>
                      </div>
                    ))}
                  </div>
                </Popup>
