...
export function TimeChart({ results }: TimeChartProps) {
  ...
  const minTime = Math.floor(Math.min(
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            ...
            if (result && ((context.dataset.label === 'Full Marathon' && result.type === 'full') ||
      ...
      y: {
        ...
        ticks: {
          callback: (value) => {
            ...
            const minutes = Math.floor(value as number % 60).toString().padStart(2, '0');
